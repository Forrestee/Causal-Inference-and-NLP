...
class ProposedModel:
    ...
    def add_placeholders(self):
        ...
        self.task_targets_ngram = tf.placeholder(tf.int64, [self.config.batch_size * 34, 5], name='task_targets_kw')
    ...
    def build_layer_op(self):
        ...
        with tf.variable_scope("proj"):
            ...
            # ngram_tuple0 = tf.concat((self.task[:, 1:], self.task[:, 0: 1] * 0), axis=1)
            # ngram_tuple1 = tf.concat((self.task[:, 2:], self.task[:, 0: 2] * 0), axis=1)
            # ngram_tuple2 = tf.concat((self.task[:, 3:], self.task[:, 0: 3] * 0), axis=1)
            # ngram_tuple3 = tf.concat((self.task[:, 4:], self.task[:, 0: 4] * 0), axis=1)
            # ngram_tuple = tf.stack((self.task, ngram_tuple0, ngram_tuple1, ngram_tuple2, ngram_tuple3), axis=2)[:, : -1]
            #
            # ngram_tuple = tf.reshape(ngram_tuple, (-1, 5))
            ...
            # ngram_loss1 = tf.reduce_mean(tf.cast(tf.equal(ngram_loss[:, 0], ngram_loss[:, 1]), dtype=tf.float32))
            # ngram_loss2 = tf.reduce_mean(tf.cast(tf.equal(ngram_loss[:, 1], ngram_loss[:, 2]), dtype=tf.float32))
            #
            # print('cross_entropy_kw:', cross_entropy_kw)
            # self.loss_task_kw = tf.reduce_mean(cross_entropy_kw) + 2. - ngram_loss1 - ngram_loss2
            # self.loss_task = self.loss_task_kw
            self.loss_task = tf.reduce_mean(cross_entropy_kw)

            # #####################
            # 距离损失

            # self.task = tf.argmax(self.intent_logits, axis=1)
            # # print('self.task:', self.task)
            # self.acc = tf.reduce_mean(tf.cast(tf.equal(self.task, self.task_targets_kw), dtype=tf.float32))
            # # cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(self.task_targets_kw, depth=3, dtype=tf.float32), logits=self.intent_logits)
            # cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(self.task_targets_kw, depth=4, dtype=tf.float32), logits=self.intent_logits)
            # self.loss_task = tf.reduce_mean(cross_entropy)
            # 计算距离
            # 对应层求距离
            # distance_loss = tf.reduce_sum(tf.square(encoder_final_state_h - encoder_final_state_h4))
            # distance_loss1 = tf.reduce_sum(tf.square(encoder_final_state_h1 - encoder_final_state_h3))
            # self.distance_loss = distance_loss + distance_loss1
            # print('*' * 100, self.loss_task, self.distance_loss)
            # self.all_loss = seta * self.loss_task + beta * self.distance_loss
