...
def inference(input_tensor_batch, n, reuse):
    ...
    with tf.variable_scope('conv0', reuse=reuse):
        ...
        conv0 = conv_bn_relu_layer(input_tensor_batch, [1, 3, 3, 3], 1)  # NEWs
        print('conv0:', conv0)
        layers.append(conv0)
    ...
    for i in range(n):
        with tf.variable_scope('conv1_%d' % i, reuse=reuse):
            if i == 0:
                # conv1 = residual_block(layers[-1], 160, first_block=True)  # CDE
                conv1 = residual_block(layers[-1], 3, first_block=True)  # NEWs
            else:
                # conv1 = residual_block(layers[-1], 160)  # CDE
                conv1 = residual_block(layers[-1], 3)  # NEWs
            layers.append(conv1)
    print('conv1:', layers[-1])

    for i in range(n):
        with tf.variable_scope('conv2_%d' % i, reuse=reuse):
            # conv2 = residual_block(layers[-1], 160)  # CDE
            conv2 = residual_block(layers[-1], 3)  # NEWs
            layers.append(conv2)

    for i in range(n):
        with tf.variable_scope('conv3_%d' % i, reuse=reuse):
            # conv3 = residual_block(layers[-1], 160)  # CDE
            conv3 = residual_block(layers[-1], 3)  # NEWs
            layers.append(conv3)
...
def tet_graph():
    input_tensor = tf.constant(np.ones([21, 3, 128, 128]), dtype=tf.float32)
